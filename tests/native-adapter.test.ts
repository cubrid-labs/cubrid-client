...
// ---------------------------------------------------------------------------
// query() — retry on transient connection error
// ---------------------------------------------------------------------------

test("NativeCubridAdapter query retries on transient error and succeeds", async () => {
  const config: ClientConfig = { ...DEFAULT_CONFIG, maxConnectionRetryCount: 2 };
  const adapter = new NativeCubridAdapter(config);

  let attempt = 0;
  const fakeCAS1 = new FakeCASConnection();
  fakeCAS1.sendAndRecv = async () => {
    throw new Error("Connection closed while reading");
  };

  const fakeCAS2 = new FakeCASConnection();
  fakeCAS2.queueResponse(buildDMLResponse(20, 1));
  fakeCAS2.queueResponse(buildSimpleResponse(0));

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const origGetOrCreate = (adapter as any).getOrCreateCAS.bind(adapter);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (adapter as any).getOrCreateCAS = () => {
    attempt++;
    if (attempt === 1) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (adapter as any).cas = fakeCAS1;
      return fakeCAS1;
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (adapter as any).cas = fakeCAS2;
    return fakeCAS2;
  };
...
test("NativeCubridAdapter query fails after exhausting retries", async () => {
  const config: ClientConfig = { ...DEFAULT_CONFIG, maxConnectionRetryCount: 1 };
  const adapter = new NativeCubridAdapter(config);

  const fakeCAS = new FakeCASConnection();
  fakeCAS.sendAndRecvError = new Error("EPIPE write failed");
...
test("NativeCubridAdapter query does not retry during transaction", async () => {
  const config: ClientConfig = { ...DEFAULT_CONFIG, maxConnectionRetryCount: 3 };
  const { adapter, fakeCAS } = createAdapterWithFakeCAS(config);

  await adapter.beginTransaction();

  fakeCAS.sendAndRecvError = new Error("Connection closed while reading");
...
test("NativeCubridAdapter query does not retry non-transient errors", async () => {
  const config: ClientConfig = { ...DEFAULT_CONFIG, maxConnectionRetryCount: 3 };
  const { adapter, fakeCAS } = createAdapterWithFakeCAS(config);

  let callCount = 0;
  fakeCAS.sendAndRecv = async () => {
    callCount++;
